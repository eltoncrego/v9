export const MUSIC_MESSAGES = {
  LANDING: {
    TITLE: 'Music',
    DESC: 'I write music in my bedroom as an extension of my journal.',
    DESC_2: "Here's a list of links to my songs.",
  },
  SONG_LINKS: [
    {
      title: 'Forever (Demo)',
      to: '/forever-demo',
    },
    {
      title: 'Lose a Friend',
      to: '/lose-a-friend',
    },
  ],
  SONG_INFO: {
    '/forever-demo': {
      TITLE: 'Forever (Demo)',
      DESC: [
        'This is a song about a serious relationship of mine that has ended.',
        "Here's a list of relevant links.",
        'Thanks for listening :)',
      ],
    },
    '/lose-a-friend': {
      TITLE: 'Lose a Friend',
      DESC: [
        'This is a song about unrequited romantic feelings.',
        "Here's a list of relevant links.",
        'Thanks for listening :)',
      ],
    },
  },
  PATH_TO_TREES: {
    '/forever-demo': [
      {
        title: 'Lyrics',
        to: '/lyrics/forever-demo',
      },
      {
        title: 'Spotify',
        href:
          'https://open.spotify.com/track/5LyscQjrwgDnxKjATb7tLC?si=Szofh0jMSnqj5NERtSbHXw',
      },
      {
        title: 'Apple Music',
        href:
          'https://music.apple.com/us/album/forever-single/1517965180?uo=4&app=music&at=1001lry3&ct=dashboard',
      },
      {
        title: 'YouTube',
        href: 'https://www.youtube.com/watch?v=3iKpOG69yeI',
      },
      {
        title: 'Amazon Music',
        href:
          'https://www.amazon.com/gp/product/B08B2SCYG9/?tag=distrokid06-20',
      },
      {
        title: 'Google Play',
        href:
          'https://play.google.com/store/music/album/Elty_Forever?id=Brurrnnd5lhr4aky3ggarrw5hre',
      },
      {
        title: 'iHeartRadio',
        href:
          'https://www.iheart.com/artist/elty-33932252/albums/forever-104397643/',
      },
      {
        title: 'Napster',
        href: 'https://us.napster.com/artist/elty/album/forever-486546730',
      },
    ],
    '/lose-a-friend': [
      {
        title: 'Lyrics',
        to: '/lyrics/lose-a-friend',
      },
      {
        title: 'Spotify',
        href:
          'https://open.spotify.com/track/3F2TFNNvnEc1jVwEzO35d0?si=130lPYmjSMW3XliZEoihyw',
      },
      {
        title: 'Apple Music',
        href:
          'https://music.apple.com/us/album/lose-a-friend-single/1519449276',
      },
      {
        title: 'YouTube',
        href: 'https://www.youtube.com/watch?v=tIvvG_REKwg',
      },
      {
        title: 'Amazon Music',
        href:
          'https://www.amazon.com/Lose-Friend-Elty/dp/B08BJ9D5JS/ref=sr_1_1?dchild=1',
      },
      {
        title: 'Google Play',
        href:
          'https://play.google.com/store/music/album?id=Bfuuon4czgv3h6royvgvea6rt2y&tid=song-Txa6z3q2kddtamluv5zakyapc5u',
      },
      {
        title: 'Napster',
        href: 'https://us.napster.com/artist/elty/album/lose-a-friend',
      },
    ],
  },
};

export const LYRICS = {
  'forever-demo': {
    title: 'Forever (Demo)',
    lyrics: [
      [
        'It takes two to make this thing work',
        'But you don’t wanna stick through the hurt',
        'And you go and push me away',
        'Even though you want me to stay and',
      ],

      [
        'You give me the silent treatment again',
        'And you shut me out only talk to your friends',
        'You’re so far away and I’m trying to close the distance',
      ],

      [
        'If I’d stayed, how long would we go',
        'Would we hate each other, I don’t even know',
        'If it was worth the effort to push for as long as we did',
        'I’m trying my best to forget you',
      ],

      [
        'No matter how hard that I try',
        'You always persist in my mind',
        'I know I should give it more time',
        'But it feels like forever',
      ],

      [
        'And I played it back in my mind',
        'Telling myself I did everything right',
        'Maybe it’s my fault',
        'Maybe I’m blind',
        'Maybe I didn’t do anything right',
      ],

      [
        'I tried to move on I saw someone new',
        'But I pushed her away because she wasn’t you',
        'I put up a wall and I can’t break it down',
        '`Cuz you’re on the other side',
      ],

      [
        'No matter how hard that I try',
        'You always persist in my mind',
        'I know I should give it more time',
        'But it feels like forever',
      ],

      [
        'And I know',
        'I shouldn’t let this be the end',
        'I’m finding myself again',
        'I’m letting you go as a friend',
        'I hope this is better',
      ],
    ],
  },
  'lose-a-friend': {
    title: 'Lose a Friend',
    lyrics: [
      [
        "I'm trying to be romantic",
        'Getting tired of all your antics',
        'You run me round in circles',
        'Trying not to hurt me',
        'But this is only making it worse',
      ],
      [
        "There's something bout our energy",
        'Cuz you were always right for me',
        'All our friends can see it',
        'They try to help you feel it',
        'You say you’re better off without me',
      ],
      [
        'We’re not good at falling in love',
        'We always try to work too hard',
        'But there’s something special we got ',
        'Going on between us',
        'But you don’t wanna take it too far',
      ],
      [
        'We stay up late and drink too much ',
        'Talk on the couch but never touch',
        "You're playing games that I can’t win",
        'You kick me out then let me in',
        "There's something 'bout the way we talk",
        'Cuz I want more but you won’t budge',
        'You’re dropping signs but ignore mine',
        'Don’t wanna lose a friend this time',
      ],
      [
        'I’m searching for a feeling',
        'Cuz you never reveal them',
        'They’re all over the place - I can never read your face',
        'I don’t know if this is something or not',
      ],
      [
        ' We’ve known each other all this time',
        'It honestly just feels right',
        'But you’re pushing me away',
        ' And you say that you’re okay',
        'Then you call me over again',
      ],
      [
        'We’re not good at falling in love',
        ' We always try to work too hard',
        'But there’s something special we got ',
        'Going on between us',
        'But you don’t wanna take it too far',
      ],
      [
        'We stay up late and drink too much ',
        'Talk on the couch but never touch',
        "You're playing games that I can’t win",
        'You kick me out then let me in',
        "There's something 'bout the way we talk",
        'Cuz I want more but you won’t budge',
        'You’re dropping signs but ignore mine',
        'Don’ t wanna lose a friend this time',
      ],
      [
        'We stay up late and drink too much ',
        'Talk on the couch but never touch',
        "You're playing games that I can’t win",
        'You kick me out then let me in',
        "There's something 'bout the way we talk",
        'Cuz I want more but you won’t budge',
        'You’re dropping signs but ignore mine',
        'Don’ t wanna lose a friend this time',
      ],
      [
        'We’re not good at falling in love',
        ' We always try to work too hard',
        'But there’s something special we got ',
        'Going on between us',
        'But you don’t wanna take it too far',
      ],
    ],
  },
};
